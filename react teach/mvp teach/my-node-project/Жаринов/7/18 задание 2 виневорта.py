...
for c in range(3):
    restored, _ = restoration.unsupervised_wiener(blur_image[:, :, c], PSF_blur, reg=None, is_real=True)

    image_deblur_perfect[:, :, c] = restored
...
for c in range(3):
    restored, _ = restoration.unsupervised_wiener(blur_image[:, :, c], PSF_deblur_perturbed, reg=None, is_real=True)

    image_deblur_perturbed[:, :, c] = restored
