...
input_image_raw = cv2.imread('text1.jpg')
# Нормировка данных изображения и перевод во float
input_image = input_image_raw.astype(np.float32) / 255.0

# Задаем искажающую маску в виде матрицы
PSF_blur = blur_mask(size=31, angle=35)
...
image_deblur[:, :, 0] = restoration.wiener(blur_image[:, :, 0], PSF_blur, noise_var)
image_deblur[:, :, 1] = restoration.wiener(blur_image[:, :, 1], PSF_blur, noise_var)
image_deblur[:, :, 2] = restoration.wiener(blur_image[:, :, 2], PSF_blur, noise_var)

# Ограничение и денормировка
image_deblur = np.clip(image_deblur, 0., 1.)
image_deblur = (image_deblur * 255.).astype(np.uint8)

# Визуализация искаженного и восстановленного изображений на одном графике
plt.figure(figsize=(10, 5))

# Подграфик для смазанного изображения
plt.subplot(1, 2, 1)
plt.imshow(cv2.cvtColor(blur_image, cv2.COLOR_BGR2RGB))
plt.title('Смазанное изображение')
plt.axis('off')

# Подграфик для восстановленного изображения
plt.subplot(1, 2, 2)
