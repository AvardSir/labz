...
def non_uniform_vertical_blur_mask(size, variation):
    ...
    k = k * (1 + variation)  # Увеличиваем значения на заданный процент
...
def create_deblur_mask(original_mask, variation):
    ...
    deblur_mask = original_mask * (1 - variation)  # Уменьшаем значения на заданный процент
...
PSF_blur = non_uniform_vertical_blur_mask(size=size, variation=0.0)  # 10% увеличение
print("Маска размытия:\n", PSF_blur)

# Размытие изображения
blur_image = cv2.filter2D(input_image, -1, PSF_blur)

# Создание маски восстановления, уменьшая значения на 10%
PSF_deblur = create_deblur_mask(PSF_blur, variation=0.1)  # 10% уменьшение
