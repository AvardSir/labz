...
d_high_factor = 0.01      # Коэффициент для размера области НЧ в спектре
threshold_value = 0.5     # Порог для бинаризации
...
_, edges_high_pass_binary = cv2.threshold(edges_high_pass, threshold_value, 1.0, cv2.THRESH_BINARY)

# Смотрим изображение на выходе
plt.figure()
plt.imshow(edges_high_pass_binary, cmap='gray')
plt.title('Фильтр высоких частот (бинаризация)')
plt.axis('off')
plt.tight_layout()
plt.show()
...
cv2.imwrite('edges_high_pass.jpg', 
            cv2.normalize((edges_high_pass_binary * 255).astype('uint8'), 
