...
f_dn1, f_up1 = 100, 150  # Низкочастотный звуковой диапазон
f_up2 = 0.3  # Очень медленные колебания
order1, order2 = 2, 2
...
for n in range(N): 
    y2[0,n] = np.exp(-6*n/N) 
y2[1,:] = list(reversed(y2[0,:])) #отзеркалили закон модуляции 
y = y1 * y2 # модуляция одного про
...
sf.write('output2.mp3', np.transpose(y), Fd)
