...
f_up3 = 2  # Очень медленные колебания
order1, order2 = 2, 4

# Создание фильтров
sos1 = signal.butter(order1, Wn=(f_dn1 / (Fd / 2), f_up1 / (Fd / 2)), btype='bandpass', output='sos')
sos2 = signal.butter(order1, Wn=(f_dn2 / (Fd / 2), f_up2 / (Fd / 2)), btype='bandpass', output='sos')
...
# Модуляция сигналов друг другом с синусоидальной функцией
modulation = np.cos(2 * np.pi * 1 * t)  # Медленная синусоидальная модуляция
y = (y1 * (1 + y2)) * (1 + y3 ) * (1 + y4)* modulation
modulation = np.sin(2 * np.pi * 1 * t)  # Медленная синусоидальная модуляция
y2 = (y1 * (1 + y2)) * (1 + y3 ) * (1 + y4)* modulation
y=y+y2
# Нормировка
