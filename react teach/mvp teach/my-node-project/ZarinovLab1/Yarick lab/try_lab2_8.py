...
def plot_amplitude_spectrum_combined(y_left, y_right):
    ...
    # Нормализация децибел так, чтобы 0 дБ было минимальным значением
    max_dB_left = np.max(S_dB_left)
    max_dB_right = np.max(S_dB_right)
    S_dB_left = max_dB_left - S_dB_left  # Инвертируем значения
    S_dB_right = max_dB_right - S_dB_right  # Инвертируем значения
    ...
    plt.xlim(2000, 5000)  # Ограничение по частоте
...
def plot_spectrogram_combined(y_left, y_right):
    ...
    D_left = np.max(D_left) - D_left  # Инвертируем значения
    ...
    plt.gca().invert_yaxis()  # Инвертирование оси Y для левого канала

    # Спектрограмма для правого канала
    D_right = librosa.amplitude_to_db(np.abs(librosa.stft(y_right)), ref=np.max)
    D_right = np.max(D_right) - D_right  # Инвертируем значения
    ...
    plt.ylim(2100, 4096)  # Ограничение по частоте
    plt.text(0, 3000, '3000 Гц', color='r', fontsize=10, ha='right')  # Добавление текста на график
    plt.gca().invert_yaxis()  # Инвертирование оси Y для правого канала
