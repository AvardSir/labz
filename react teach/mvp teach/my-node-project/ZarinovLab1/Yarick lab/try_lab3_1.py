...
librosa.display.waveshow(y[0, :], sr=sr, color='b', ax=ax, label='Левый канал (синусоидальная модуляция)')
librosa.display.waveshow(y[1, :], sr=sr, color='r', ax=ax, label='Правый канал (косинусоидальная модуляция)')
ax.label_outer()
ax.legend()
ax.grid()
plt.xlabel('Время')
