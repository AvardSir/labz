...
def okumura_hata_loss(d, urban_factor=0):
    # Коррекция для высоты мобильной станции
    a_hMS = (1.1 * np.log10(f0) - 0.7) * h_MS - (1.56 * np.log10(f0) - 0.8)
    
    # Формула потерь с учетом коррекционного коэффициента для больших городов
    L = (46.3 + 33.9 * np.log10(f0) - 13.82 * np.log10(h_BS) - 
         a_hMS + (44.9 - 6.55 * np.log10(h_MS)) * np.log10(d) + urban_factor)
...
for key in total_sum_D:
    print(f"Тип: {key}")
    print(f"  - Суммарная скорость: {total_sum_D[key][-1]:.2f} Мбит/с")
